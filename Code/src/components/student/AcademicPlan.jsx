...
const AcademicPlan = () => {
  ...
  const tabs = [
    ...
    { id: 'remaining', label: 'Remaining', count: plan.remainingCourses?.length || 0, icon: '⏳' },
    { id: 'dropped', label: 'Dropped', count: plan.droppedCourses.length, icon: '❌' }
  ];

  const courses = activeTab === 'current' ? plan.currentCourses :
    activeTab === 'completed' ? plan.completedCourses :
      activeTab === 'remaining' ? (plan.remainingCourses || []) : plan.droppedCourses;
              ...
              <div key={course.enrollment_id} className="course-card">
                ...
                <div className="course-status">
                  ...
                  <span
                    ...
                    style={{
                      background: `${getStatusColor(course.status || 'remaining')}15`,
                      color: getStatusColor(course.status || 'remaining')
                    }}
                  >
                    {course.status ? course.status.replace('_', ' ') : 'Remaining'}
                  </span>
                </div>
              </div>
            ))}
